...
object ParallelCountChange {
  ...
  def moneyThreshold(startingMoney: Int): Threshold = (money: Int, coins: List[Int]) => (money <= ((2 * startingMoney) / 3))

  /** Threshold heuristic based on the total number of initial coins. */
  def totalCoinsThreshold(totalCoins: Int): Threshold = (money: Int, coins: List[Int]) => (coins.length <= ((2 * totalCoins) / 3))
